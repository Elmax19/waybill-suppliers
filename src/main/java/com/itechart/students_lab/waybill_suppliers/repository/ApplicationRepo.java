...
public interface ApplicationRepo extends JpaRepository<Application, Long> {
    ...
    @Query(value = "update application join warehouse w on w.id = application.warehouse_id " +
            "join customer c on c.id = w.customer_id " +
            "set updating_user_id=?2, last_update_date_time=now() where customer_id=?3 and number=?4", nativeQuery = true)
    void updateApplicationData(Long updatingUserId, Long customerId, int number);
}
