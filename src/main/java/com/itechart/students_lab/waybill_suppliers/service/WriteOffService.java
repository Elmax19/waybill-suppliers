...
import com.itechart.students_lab.waybill_suppliers.repository.WriteOffItemRepo;
...
public class WriteOffService {
    ...
    private final WriteOffItemRepo writeOffItemRepo;
    ...
    public WriteOffDto create(WriteOffDto writeOffDto) {
        ...
        int placeUnits = 0;
        Long id = writeOffRepo.getMaxId() + 2;
        writeOff.setId(id);
        writeOffRepo.create(writeOff.getId(), writeOff.getDateTime(), writeOff.getCreatingUser().getId(), writeOff.getWarehouse().getId(), writeOff.getCar().getId());
        writeOff.getWriteOffItems().forEach(item -> item.setWriteOff(writeOff));
        writeOff.getWriteOffItems().forEach(item -> {
            writeOffItemRepo.save(item);
            if (writeOffDto.getWarehouseId() != null) {
                warehouseItemRepo.updateWarehouseItemCount(writeOffDto.getWarehouseId(), item.getItem().getId(), (-item.getAmount()));
            }
        });
